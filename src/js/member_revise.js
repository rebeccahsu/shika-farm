...
function cancel_btn(ID) {
	if ($(`#sort${ID}`).find(".cel_list_btn").hasClass('canceled')){
  ...
  console.log(ID);
	fetch("./php/mb_orderCancel.php", {
		...
		.then((res) => {
			...
			$(`#sort${ID}`).find(".cel_list_btn").addClass("canceled");
		});
}
				...
				.then((res) => {
					if (res.successful) {
						$(e.target)
							.closest(".act_list")
							.find(".cancel_btn")
							.css({ "background-color": "gray", border: "none" });
...
fetch("./php/mb_list.php", {
	...
	.then((res) => {
		...
		let tr_row = "";
		for (let i = 0; i < res.length; i++) {
      tr_row += "<tr class='li2' id='sort" + res[i].ID + "'>";
      tr_row += "<td>" + res[i].ORDER_DATE + "</td>";
      tr_row += "<td>" + res[i].ID + "</td>";
        ...
        tr_row += "<button class='cel_list_btn canceled'>取消訂單</button>";
      }
      else{
        tr_row += "<button class='cel_list_btn' onclick='cancel_btn(" + res[i].ID + ")'>取消訂單</button>";
      }
			// tr_row += "<button class='cel_list_btn' onclick='cancel_btn(" + res[i].ID + ")' value='" + (2 * parseInt(i) + 1) + "'>取消訂單</button>";
			tr_row +=
				"<button id='op_btn' onclick='open_detail(" +
				res[i].ID +
				")'>+</button>";
			tr_row += "</td>";
			tr_row += "</tr>";
			tr_row +=
				"<tr class='li3' id='li" + res[i].ID + "' style='display:none;'>";
			tr_row += "</tr>";
		}
